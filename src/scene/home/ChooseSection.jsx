    ...
    <>
      <div className="my-6">
        <div className="flex flex-col">
          <h1 className="text-xl font-bold">Benefit of choosen NPRGS</h1>
          <p className="text-sm sm:text-lg  font-semibold">
            Our program offers numerous benefits to help you succeed in your
            career
          </p>
          <div className="flex flex-wrap py-4 gap-8 w-full">
            {chooseCards.map((item, i) => (
              <div key={i} className="">
                <ChooseCard icon={item.icon} head={item.head} text={item.text} />
              </div>
            ))}
          </div>
        </div>
